...
## This script runs an SECC in 5GHz band
...
sed -i -e "s/vendor_elements.*/vendor_elements=${VSE}/" hostapd5.conf
sed -i -e "s/assocresp_elements.*/assocresp_elements=${VSE}/" hostapd5.conf

# run hostapd using configuration file
sudo hostapd hostapd5.conf -B
